...
namespace Bandi_Complex
{
    ...
    class BandiCrop_Analyzer
    {
        static void Main(string[] args)
        {
            ...
            MWStructArray env_status = new MWStructArray();
            MWStructArray control_actions = new MWStructArray();
            ...
            double temp = 4;
            double hum = 80;
            env_status = (MWStructArray)crop_env_checker(crops_db, temp, hum, crop_name);
            ...
            double temp_out = -3;
            double hum_out = 40;
            control_actions = (MWStructArray)crop_env_controller(crops_db, crop_name, temp_in, hum_in, temp_out, hum_out);

            // TEST DONG OBJECT TO APPLY CONTROLS
            Dong dong1 = new Dong(1)
            {
                Name = "NARE_DONG",
                Fan_total = 5,
            };
            ...
            execute_control(control_actions, dong1);
        ...
        // METHOD INTERFACING MODULE TO HARDWARE-LEVEL COMMANDS
        public static void execute_control(MWStructArray control_action, object dong)
        {
            Console.WriteLine("\n======================= SENDING CONTROL SIGNALS =======================");

            // PARASE DONG INFORMATION
            ...
            {
                Console.WriteLine(field_names[ix] + " : " + control_action.GetField(field_names[ix]));

                string target = "fan"; // EYESEN ACTUATORS: switch, fan, relay 입력 가능
                int dongNumber = 0;
                int switchNumber = 0; // NON-EYESEN ACTUATORS
                string command = "on"; //  switch -> stop, open, close / (2) fan -> on, off / (3) on, off)
                //EyesenInterface.EquipControl(target, dongNumber, switchNumber, command);
            }
        }
